...
<div class="container px-0 ">
    <div class="card  box-shadow-0">
        ...
        <div class="card-body pt-0">
                ...
                <div class="card-body p-4">
                    <form id="formReserva" autocomplete="off" novalidate>
                        ...
                        <div class="form-group col-12 col-md-4">
                            ...
                            <small class="form-text text-muted">Caso seja um grupo, deve ser selecionado o responsável.</small>
                        </div>  
            ...
            <!-- Painel de reservas do dia selecionado -->
            <div class="card ">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h2 class="h5 mb-0">Reservas do dia <span id="dayTitle" class="text-secondary"></span></h2>
                        <span class="badge badge-info" id="badgeCapacity"></span>
                    </div>
                    <div class="table-responsive " style="max-height: 320px;">
                        <table class="table table-striped table-hover align-middle mb-0">
                            ...
                            <tbody id="dayReservationsBody">
                                <tr>
                                    <td colspan="5" class="text-center text-secondary">Nenhuma reserva ainda.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
<style>
    .card { border: none; border-radius: 1rem; box-shadow: 0 10px 25px rgba(0,0,0,.25); }
    .group-header { background: rgba(0,0,0,.04); border: 1px solid rgba(0,0,0,.08); border-radius: 1rem; }
    .dark-theme .group-header { background: rgba(255,255,255,.04); border: 1px solid rgba(255,255,255,.08); border-radius: 1rem; }
    .table-sticky thead th { position: sticky; top: 0; background: #fff; z-index: 1; }

    /* Slots */
    .slot-btn { position: relative; min-width: 110px; }
    .slot-btn.disabled.btn-outline-danger { color: rgb(121, 135, 161); }
    .slot-badge { position: absolute; top: -6px; right: -6px; font-size: .75rem; }
    .slot-btn.active-start.btn-outline-light:not(:disabled):not(.disabled).active { outline: 2px solid #0d6efd; }
    .slot-btn.active-end.btn-outline-light:not(:disabled):not(.disabled).active { outline: 2px dashed #0d6efd; }
    .slot-btn.in-range { background: rgba(25,135,84,.2); border-color: rgba(25,135,84,.5); }
    .slot-btn.conflict { background: rgba(220,53,69,.25) !important; border-color: rgba(220,53,69,.8) !important; }

    /* Helpers para espaçamento (equivalente ao gap) */
    .flex-gap-2 > * { margin-right: .5rem; margin-bottom: .5rem; }
    .mr-3-imp { margin-right: 1rem !important; }
    ...
    .resource-col { display: block; }
    @media (max-width: 991.98px) {
        .resource-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
    }
    @media (max-width: 575.98px) {
        .resource-grid { grid-template-columns: repeat(1, minmax(0, 1fr)); }
    }
    ...
    .resource-empty { padding: .75rem; }

    /* 1) o dialog ocupa a janela (descontando as margens padrão do BS4) */
#modalTematica .modal-dialog {
  margin: 1.75rem auto;                /* mantém margens do BS >=576px */
  height: calc(100vh - 3.5rem);        /* 100vh - (1.75rem * 2) */
}
...
#modalTematica .modal-body {
  ...
  display: flex;
  flex-direction: column;
  min-height: 0;        /* crucial pra permitir que os filhos usem 100% */
  overflow: hidden;     /* evita barra dupla */
}
...
#modalTematica .iframe-wrap {
  flex: 1 1 auto;
  min-height: 0;        /* sem isso, o iframe não consegue h-100 */
  display: flex;
}
...
?>
  ...
  const SPECIAL_CLOSED_INTERVALS_BY_DATE = { /* '2025-09-22': [ { start: '16:00', end: '18:00' } ] */ };
  const SPECIAL_OPENINGS_BY_DATE = { /* '2025-09-20': [ { start: '09:00', end: '13:00' } ] */ };
  ...
  function minutesToHHMM(total) { const h = Math.floor(total/60), m = total%60; return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`; }
  function todayStr() { const d = new Date(); d.setHours(0,0,0,0); return d.toISOString().slice(0,10); }
  function isoToDate(iso) { const [y,mo,da] = iso.split('-').map(Number); return new Date(y,mo-1,da); }
  function isoWeekday(iso) { let wd = isoToDate(iso).getDay(); return wd===0?7:wd; }
  ...
  function isOverlapping(aStart, aEnd, bStart, bEnd){ return aStart < bEnd && bStart < aEnd; }
  ...
  function generateGroupedSlots(isoDate){
    ...
    openings.forEach(win => {
      const wStart = toMinutes(win.start), wEnd = toMinutes(win.end);
      let current = [];
      for (let t = wStart; t + SLOT_MINUTES <= wEnd; t += SLOT_MINUTES){
        const s = t, e = t + SLOT_MINUTES;
        ...
        if (blocked){
          if (current.length){
            ...
            groups.push({ label, slots: current });
            current = [];
          }
          continue;
        }
        const used = peopleInInterval(isoDate, s, e);
        current.push({ start: minutesToHHMM(s), end: minutesToHHMM(e), used, left: Math.max(0, CAPACITY - used) });
      }
      if (current.length){
        const last = current[current.length - 1];
        const label = `${current[0].start}–${minutesToHHMM(toMinutes(last.start) + SLOT_MINUTES)}`;
  ...
  function peopleInInterval(isoDate, startMin, endMin){
    return reservations.filter(r => r.date === isoDate).reduce((sum, r) => {
      const rs = toMinutes(r.start), re = rs + Number(r.duration);
  ...
  function canBookRange(isoDate, group, startIdx, endIdx, people){
    if (startIdx == null) return { ok:false, reason:'Selecione a entrada.' };
    ...
    for (let i = startIdx; i < effectiveEndIdx; i++){
      ...
      if (slot.used + Number(people) > CAPACITY){
        return { ok:false, reason:`Lotação excedida entre ${slot.start}-${slot.end}.` };
      }
    }
    return { ok:true };
  }
  ...
  function renderResources(filterText){
    ...
    filtered.forEach((r) => {
      const col = document.createElement('div'); col.className = 'resource-col';
      const id = `res-${r.id}`;
  ...
  function getActivityPayload(){
    ...
    if (type === 'oficina'){
      const activity = getSelectedOficina();
      if (!activity) return { ok:false, reason:'Selecione uma oficina.' };
      return { ok:true, value: { type:'oficina', id: activity.id } };
    }
    // livre
    ...
    return { ok:true, value: { type:'livre', description, resourceIds } };
  }

  function getActivityResourceIds(actValue){
    if (!actValue) return [];
    if (actValue.type === 'oficina'){ const a = ACTIVITIES.find(x=>x.id===actValue.id); return a ? a.resourceIds : []; }
    if (actValue.type === 'livre'){ return actValue.resourceIds || []; }
    return [];
  }
  ...
  function toggleActivityBlocks(){
    const type = document.querySelector('input[name="actType"]:checked').value;
    if (type == 'oficina'){ oficinaBlock.classList.remove('d-none'); livreBlock.classList.add('d-none'); }
    else { oficinaBlock.classList.add('d-none'); livreBlock.classList.remove('d-none'); }
  }
  ...
  function reservationResourceIds(activity){
    ...
    if (activity.type === 'oficina'){
      const a = ACTIVITIES.find(x=>x.id===activity.id); return a ? a.resourceIds : [];
    }
    if (activity.type === 'livre'){ return activity.resourceIds || []; }
    return [];
  }
  ...
  const dayReservationsBody = document.getElementById('dayReservationsBody');
  const dayTitle = document.getElementById('dayTitle');
  const badgeCapacity = document.getElementById('badgeCapacity');
  ...
  const participanteNameInput = document.getElementById('Participante_id_Text');
  ...
  datePicker.min = today; datePicker.max = maxIso; datePicker.value = today;
  ...
  function renderGroups({ keepSelections } = { keepSelections: true }){
    const iso = selectedDate;
    const openings = getOpenIntervalsForDate(iso);
    dayTitle.textContent = `(${new Intl.DateTimeFormat('pt-BR',{dateStyle:'full'}).format(isoToDate(iso))})`;

    slotGroupsEl.innerHTML = '';
    if (!keepSelections) selectedRanges = {};

    if (iso < todayStr()) { slotsHelper.textContent = 'Data anterior: sem horários.'; currentGroups = []; updateSelectionSummary([]); return; }
    if (CLOSED_DATES.has(iso)) { slotsHelper.textContent = 'Fechado o dia todo (feriado/agenda).'; currentGroups = []; updateSelectionSummary([]); return; }
    if (!openings.length) { slotsHelper.textContent = 'Dia sem funcionamento.'; currentGroups = []; updateSelectionSummary([]); return; }

    applyTodayPastIntervals();
    currentGroups = generateGroupedSlots(iso);
    if (!currentGroups.length){ slotsHelper.textContent = 'Sem horários disponíveis (fechamentos parciais ou sem janelas úteis).'; updateSelectionSummary([]); return; }
    ...
    currentGroups.forEach((group, gi) => {
      ...
      group.slots.forEach((slot, si) => {
        ...
        btn.type = 'button'; btn.className = 'btn btn-outline-light slot-btn';
        ...
        if (slot.left <= 0){ btn.classList.add('disabled','btn-outline-danger'); badge.classList.remove('badge-secondary'); badge.classList.add('badge-danger'); }
        else if (slot.left <= CAPACITY*0.25){ btn.classList.add('btn-outline-warning'); badge.classList.remove('badge-secondary'); badge.classList.add('badge-warning'); }
        else { btn.classList.add('btn-outline-success'); badge.classList.remove('badge-secondary'); badge.classList.add('badge-success'); }
      ...
      card.appendChild(title); card.appendChild(wrap); slotGroupsEl.appendChild(card);
    });
  ...
  function collectSelections(){
    ...
    Object.entries(selectedRanges).forEach(([giStr, r]) => {
      ...
      out.push({ gi, start, end, duration, groupLabel: g.label, group: g });
    });
  ...
  function updateSelectionSummary(groups){
    ...
    if (!selections.length){ selectionSummary.innerHTML = activityStr; btnReserve.disabled = true; return; }
  ...
  function updateDayTable(){
    const iso = selectedDate;
    const list = reservations.filter(r => r.date === iso).sort((a,b)=>toMinutes(a.start)-toMinutes(b.start));
    const body = dayReservationsBody;
    body.innerHTML = '';
    if (!list.length){
      body.innerHTML = '<tr><td colspan="5" class="text-center text-secondary">Nenhuma reserva ainda.</td></tr>';
    } else {
      for (const r of list){
        const tr = document.createElement('tr');
        const end = minutesToHHMM(toMinutes(r.start) + Number(r.duration));
        tr.innerHTML = `
          <td><span class="badge badge-secondary">${r.start}</span></td>
          <td>${end}</td>
          <td>${r.people}</td>
          <td>${r.name || '-'}</td>
    ...
    const peak = computePeakLoad(iso);
    badgeCapacity.textContent = `Capacidade: ${CAPACITY} • Pico atual: ${peak}/${CAPACITY}`;
    badgeCapacity.className = 'badge ' + (peak>=CAPACITY ? 'badge-danger' : peak>=CAPACITY*.75 ? 'badge-warning' : 'badge-success');
  }

  function computePeakLoad(iso){
    const opens = getOpenIntervalsForDate(iso); if (!opens.length) return 0;
    let peak = 0;
    opens.forEach(win=>{
      for (let t = toMinutes(win.start); t < toMinutes(win.end); t += SLOT_MINUTES){
        const used = peopleInInterval(iso, t, Math.min(t+SLOT_MINUTES, toMinutes(win.end)));
  ...
  peopleInput.addEventListener('input', ()=>{ toggleEscolaSection(); refreshSelectionStyles(); updateSelectionSummary(currentGroups); });

  if (turmaEscolaSelect){
    turmaEscolaSelect.addEventListener('change', ()=>{ toggleTurmaExtra(turmaEscolaSelect.value === '0'); });
  }
  ...
  document.querySelectorAll('input[name="actType"]').forEach(el=>{
    el.addEventListener('change', ()=>{ toggleActivityBlocks(); updateSelectionSummary(currentGroups); });
  });
  ...
  $('.btnModalDetalhes').on('click', function(evt){
    ...
    let url = '<?= base_url(); ?>OficinaTematica/descricao/'+idOficina;
  ...
  function applyTodayPastIntervals() {
    ...
    SPECIAL_CLOSED_INTERVALS_BY_DATE[iso].push({ start: '00:00', end: minutesToHHMM(endMin) });
  }
  ...
  setInterval(() => {
    ...
    .then(data => {
        ...
        renderGroups({ keepSelections: true });
        updateDayTable();
    })
    .catch(error => {
        console.error("Erro na requisição:", error);
