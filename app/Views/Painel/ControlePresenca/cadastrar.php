...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">Controle Presença</h4><span class="text-muted mt-1 tx-13 ml-2 mb-0">/ Cadastrar</span>
        </div>
    </div>
</div>
<!-- breadcrumb -->
...
<div class="container px-0 ">
    <div class="card  box-shadow-0">
        ...
        <div class="card-body pt-0">
            <form id='formCadastrar' action="<?PHP echo base_url('ControlePresenca/doCadastrar'); ?>" class="needs-validation" method="post" enctype="multipart/form-data" >
                <div class="form-row">
                    
                    <div class="form-group col-12 col-md-6">
                        <label class="main-content-label tx-11 tx-medium tx-gray-600">Evento</label> 
                        <div class="input-group">
                            <input class="form-control" name="Evento_id_Text" id="Evento_id_Text" type="text" disabled="true" onclick="$('#addonSearchEvento_id').click()" value="<?= old('Evento_id_Text'); ?>"/>
                            ...
                            <input class="d-none" name="Evento_id" id="Evento_id" type="text" value="<?= old('Evento_id'); ?>" />
                        </div>
                    </div>                    <div class="form-group col-12 col-md-6">
                        <label class="main-content-label tx-11 tx-medium tx-gray-600">Descrição</label> 
                        <input class="form-control" name="descricao" id="descricao" type="text" maxlength="100" value="<?= old('descricao') ?>">
                    </div>                                        
                    ...
                    <div class="form-row px-2">
                        <div class="form-group col-auto">
                            ...
                            <div class="input-group mb-3">
                                <input class="form-control" name="presencaevento_ParticipanteEvento_id_Text" id="presencaevento_ParticipanteEvento_id_Text" type="text" disabled="true" onclick="$('#addonSearchpresencaevento_ParticipanteEvento_id').click()" value="<?= old('presencaevento_ParticipanteEvento_id_Text'); ?>"/>
                                <div class="input-group-append">
                                ...
                                <input class="d-none" name="presencaevento_ParticipanteEvento_id" id="presencaevento_ParticipanteEvento_id" type="text" value="<?= old('presencaevento_ParticipanteEvento_id'); ?>" />
                            </div>
                        </div>                        
                        <div class="form-group col-auto">
                            ...
                            <select class="form-control" name="presencaevento_presente" id="presencaevento_presente" required="" >
                                <option value="" <?= old('presencaevento_presente')=='' ? 'selected' : ''; ?>></option>
                                <?PHP foreach (App\Entities\PresencaEventoEntity::_op('presente') as $k => $op){ ?>
                                <option value="<?= $k; ?>" <?= old('presencaevento_presente') === $k ? 'selected' : ''; ?>><?= $op; ?></option>
                                <?PHP } ?>
                            </select>
                        </div>                        
                        <div class="form-group col-auto">
...
<script>
    ...
    function addPresencaEvento() {
        ...
        html = html.replaceAll('{_ParticipanteEvento_id_}', $('#presencaevento_ParticipanteEvento_id').val());
        html = html.replaceAll('{_ParticipanteEvento_id_Text_}', $('#presencaevento_ParticipanteEvento_id_Text').val());
        html = html.replaceAll('{_presente_}', $('#presencaevento_presente').val());
        html = html.replaceAll('{_presente_Text_}', $('#presencaevento_presente option:selected').text());
