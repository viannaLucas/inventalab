                ...
                <div class="row">
                    ...
                    <fieldset class="border rounded-10 m-0 mb-3 p-2 w-100">
                        <div class="border-bottom mx-n1 mb-3">
                            ...
                            <p class="px-3 text-muted mb-2">Cadastre cada período em que o evento ocupará o espaço físico. É possível adicionar mais de um dia, inclusive com horários distintos.</p>
                        </div>
...
<template id="templateRowReservaEspaco">
    <tr id='ReservaEspaco_{_index_}'>
        ...
        <td>
            <input type="hidden" class="ignoreValidate" name="ReservaEspaco[{_index_}][diaInteiro]" value="{_diaInteiroValor_}" />
            <input type="text" class="form-control ignoreValidate" readonly="true" value="{_diaInteiroTexto_}" />
        </td>
        <td>
...
<script>
    ...
    function addReservaEspaco() {
        ...
        html = html.replaceAll('{_data_}', data);
        html = html.replaceAll('{_horaInicio_}', horaInicio);
        html = html.replaceAll('{_horaFim_}', horaFim);
        html = html.replaceAll('{_diaInteiroValor_}', diaInteiro ? '1' : '0');
        html = html.replaceAll('{_diaInteiroTexto_}', diaInteiro ? 'Sim' : 'Não');
    ...
    function verificarDataHoraReservas(){
        ...
        $('#listTableReservaEspaco tbody tr').each(function(index) {
            ...
            reservas.push({
                ...
                diaInteiro: $row.find('input[type="hidden"][name^="ReservaEspaco"][name$="[diaInteiro]"]').val() || '0'
            });
        });
